...
for x in sizes:
    training_features, testing_features, training_target, testing_target = \
                train_test_split(X, y,train_size=x,test_size=0.2,random_state=42)
    exported_pipeline.fit(training_features, training_target)
    starttime = time.perf_counter_ns()
    results = exported_pipeline.predict(testing_features)
    endtime = time.perf_counter_ns()
    times.append(((endtime-starttime)/1000000000)/len(testing_target))    
    scores.append(exported_pipeline.score(testing_features,testing_target))
    mses.append(metrics.mean_squared_error(testing_target,results))
sns.set()
fig, (ax1, ax2) = plt.subplots(1, 2)
fig.suptitle('Performance of Model per Training Set Size (with constant testing set size)')
ax1.plot(sizes*6908, scores)
ax1.set(xlabel='Training Set Size',ylabel='Model Score')
ax2.plot(sizes*6908, mses)
ax2.set(xlabel='Training Set Size',ylabel='Model MSE')

print(pd.DataFrame(sizes*6908,times))
